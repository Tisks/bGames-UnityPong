...
public class JSONObjectTester : MonoBehaviour {
  ...
  ""taglib"": {
	...
	void Start() {
		...
		//JSONObject2 usage example:

		//Parse string into a JSONObject2:
		JSONObject2 JSONObject2 = JSONObject2.Parse(stringToEvaluate);
		if (JSONObject2 == null) { //Just to shut up Unity's 'unused variable' warning
		}

		//You can also create an "empty" JSONObject
		JSONObject2 emptyObject = new JSONObject2();
		...
		//JSONObject2's can also be created using this syntax:
		JSONObject2 newObject = new JSONObject2 {
			{"key", "value"},
			{"otherKey", 123},
			{"thirdKey", false}
		};

		//JSONObject2 overrides ToString() and outputs valid JSON
		Debug.Log("newObject.ToString() -> " + newObject.ToString());

		//JSONObject2s support array accessors
	...
	void OnGUI() {
		...
		if (GUI.Button(new Rect(Screen.width - 150, 10, 145, 75), "Evaluate JSON")) {
			var JSONObject3 = JSONObject2.Parse(stringToEvaluate);
			if (JSONObject3 == null) {
				Debug.LogError("Failed to parse string, JSONObject2 == null");
			} else {
				Debug.Log("Succesfully created JSONObject2");
				Debug.Log(JSONObject3.ToString());
			}
		}
